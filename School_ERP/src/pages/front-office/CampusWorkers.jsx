    ...
    <div className="flex flex-col h-full bg-[#f4f6f8] overflow-y-auto">
        ...
        <div className="flex justify-between items-start">
          ...
          <div className="flex gap-3">
            <button 
              ...
              className="bg-white border border-gray-300 text-gray-700 px-4 py-2 rounded-none text-[13px] font-bold hover:bg-gray-50 shadow-sm flex items-center gap-2 transition-colors"
            >
      ...
      <div className="p-8 w-full">
        ...
        <div className="flex justify-between items-center mb-4">
          <div className="flex items-center gap-1 bg-white p-1 rounded-none border border-gray-200 shadow-sm flex-wrap">
          ...
          <div className="p-4 border-b border-gray-100 flex flex-wrap gap-4 items-center justify-between">
            <div className="flex flex-wrap items-center gap-4">
              ...
              <div className="flex items-center gap-2">
                ...
                <button onClick={() => window.print()} className="border border-gray-300 bg-white text-gray-600 px-3 py-1.5 rounded-none hover:bg-gray-50 text-[12px] font-bold flex items-center gap-1.5 shadow-sm">
                  ...
                  <tr key={w.id} className="border-b border-gray-100 hover:bg-gray-50">
                    ...
                    <td className="py-3 px-4 border-r border-gray-100">
                      {w.policeCheck === 'Not verified' ? (
                        <span className="bg-[#ffc107] text-gray-900 text-[11px] font-bold px-2 py-0.5 rounded-none shadow-sm">Not verified</span>
                      ) : (
                        <span className="bg-[#28a745] text-white text-[11px] font-bold px-2 py-0.5 rounded-none shadow-sm">Verified</span>
                      )}
                    </td>
                    <td className="py-3 px-4 border-r border-gray-100">
                      {w.state === 'Expired' && <span className="bg-[#dc3545] text-white text-[11px] font-bold px-2 py-0.5 rounded-none shadow-sm">{w.state}</span>}
                    </td>
                    <td className="py-3 px-4 text-center">
                      <div className="flex justify-center items-center gap-1.5">
                        <button 
                          onClick={() => navigate(`/front-office/campus-workers/view/${w.id}`)}
                          className="bg-[#17a2b8] hover:bg-[#138496] text-white p-1 rounded-none shadow-sm transition-colors" title="View"
                        >
                          <Eye className="w-3.5 h-3.5" />
                        </button>
                        <button 
                          onClick={() => navigate(`/front-office/campus-workers/edit/${w.id}`)}
                          className="bg-[#fd7e14] hover:bg-[#e86f0b] text-white p-1 rounded-none shadow-sm transition-colors" title="Edit"
                        >
                          <Edit className="w-3.5 h-3.5" />
                        </button>
                        <button 
                          onClick={() => handleDelete(w.id)}
                          className="bg-[#dc3545] hover:bg-[#c82333] text-white p-1 rounded-none shadow-sm transition-colors" title="Delete"
                        >
