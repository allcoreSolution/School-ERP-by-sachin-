...
const AddIncome = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      const res = await accountantService.addIncome({
        date: form.date,
        amount: Number(form.amount) || 0,
        amountPaid: Number(form.amount) || 0, // In income simulation via fees collection
        description: form.description,
        paymentMode: form.mode,
        head: form.head
      });
      if(res.success || res) {
        Swal.fire({ title: 'Success', text: 'Income added successfully', icon: 'success' });
      ...
      <div className="bg-white rounded-[3px] border-t-[3px] border-t-[#3c8dbc] shadow-sm max-w-4xl mx-auto">
        ...
        <form className="p-5" onSubmit={handleSubmit}>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4 text-[14px]">
            ...
            <div className="flex flex-col gap-1.5 focus-within:text-[#3c8dbc]">
              ...
              <div className="relative">
                ...
                <select value={form.head} onChange={e=>setForm({...form, head: e.target.value})} className="w-full border border-[#d2d6de] rounded-[3px] pl-9 pr-3 py-1.5 bg-white focus:outline-none focus:border-[#3c8dbc] appearance-none" required>
            ...
            <div className="flex flex-col gap-1.5 focus-within:text-[#3c8dbc]">
              ...
              <input type="text" value={form.name} onChange={e=>setForm({...form, name: e.target.value})} placeholder="Enter person or entity name" className="w-full border border-[#d2d6de] rounded-[3px] px-3 py-1.5 bg-white focus:outline-none focus:border-[#3c8dbc]" required />
            </div>
            ...
            <div className="flex flex-col gap-1.5 focus-within:text-[#3c8dbc]">
              ...
              <div className="relative">
                ...
                <input type="date" value={form.date} onChange={e=>setForm({...form, date: e.target.value})} className="w-full border border-[#d2d6de] rounded-[3px] pl-9 pr-3 py-1.5 bg-white focus:outline-none focus:border-[#3c8dbc]" required />
              </div>
            </div>

            <div className="flex flex-col gap-1.5 focus-within:text-[#3c8dbc]">
              <label className="font-semibold text-[#333]">Amount (₹) *</label>
              <input type="number" value={form.amount} onChange={e=>setForm({...form, amount: e.target.value})} placeholder="0.00" min="0" step="0.01" className="w-full border border-[#d2d6de] rounded-[3px] px-3 py-1.5 bg-white focus:outline-none focus:border-[#3c8dbc]" required />
            </div>
            ...
            <div className="flex flex-col gap-1.5 focus-within:text-[#3c8dbc]">
              ...
              <div className="relative">
                ...
                <select value={form.mode} onChange={e=>setForm({...form, mode: e.target.value})} className="w-full border border-[#d2d6de] rounded-[3px] pl-9 pr-3 py-1.5 bg-white focus:outline-none focus:border-[#3c8dbc] appearance-none">
            ...
            <div className="flex flex-col gap-1.5 md:col-span-2 focus-within:text-[#3c8dbc]">
              ...
              <div className="relative">
                ...
                <textarea rows="3" value={form.description} onChange={e=>setForm({...form, description: e.target.value})} placeholder="Enter income details..." className="w-full border border-[#d2d6de] rounded-[3px] pl-9 pr-3 py-2 bg-white focus:outline-none focus:border-[#3c8dbc] resize-y"></textarea>
              </div>
            </div>
          ...
          <div className="mt-8 flex justify-end gap-3 bg-[#fdfdfd] -mx-5 -mb-5 px-5 py-4 border-t border-gray-100">
            ...
            <button type="submit" disabled={loading} className="px-6 py-2 text-[14px] bg-[#3c8dbc] text-white rounded-[3px] hover:bg-[#367fa9] flex items-center justify-center gap-2 shadow-sm transition-colors disabled:opacity-70">
              <Save className="w-4 h-4" /> {loading ? 'Saving...' : 'Save Income'}
            </button>
          </div>
        </form>
      </div>
