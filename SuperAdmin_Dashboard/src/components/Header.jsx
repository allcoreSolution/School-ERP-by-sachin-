...
import { Menu, Settings, Globe, Bell, Search, ChevronDown, Sparkles, X, LogOut, User as UserIcon } from 'lucide-react';
...
const Header = ({ onMenuClick }) => {
  ...
  return (
    <header className="h-16 bg-white border-b border-gray-100 flex items-center justify-between px-4 md:px-6 fixed top-0 right-0 left-0 lg:left-64 z-10 shadow-sm">
        ...
        <div className="relative">
          <button
            ...
            <div className="hidden sm:block text-left">
              <p className="text-xs font-bold text-gray-800 leading-tight">{user?.name || 'Super Admin'}</p>
          ...
          {showUserMenu && (
            <div className="absolute right-0 top-12 w-48 bg-white rounded-none shadow-2xl border border-gray-100 overflow-hidden z-50">
              <div className="px-4 py-3 border-b border-gray-50 flex items-center gap-3">
                 <div className="flex-1 min-w-0">
                    <p className="text-xs font-bold text-gray-800 truncate">{user?.name || 'Super Admin'}</p>
                    <p className="text-[10px] text-gray-500 truncate">{user?.email || 'admin@erp.com'}</p>
                 </div>
              </div>
              <div className="py-1">
                <button onClick={() => { setShowUserMenu(false); navigate('/profile'); }} className="w-full text-left px-4 py-2 text-xs text-gray-700 hover:bg-orange-50 hover:text-orange-600 flex items-center gap-2">
                  <UserIcon className="w-3.5 h-3.5" /> My Profile
                </button>
                <button onClick={() => { setShowUserMenu(false); navigate('/settings'); }} className="w-full text-left px-4 py-2 text-xs text-gray-700 hover:bg-orange-50 hover:text-orange-600 flex items-center gap-2">
                  <Settings className="w-3.5 h-3.5" /> Settings
                </button>
                <div className="border-t border-gray-50 my-1"></div>
                <button onClick={() => { setShowUserMenu(false); logout(); navigate('/login'); }} className="w-full text-left px-4 py-2 text-xs text-red-600 hover:bg-red-50 flex items-center gap-2">
      ...
      {/* Click outside to close notif or userMenu */}
      {(showNotif || showUserMenu) && <div className="fixed inset-0 z-40" onClick={() => { setShowNotif(false); setShowUserMenu(false); }} />}
    </header>
  );
};
