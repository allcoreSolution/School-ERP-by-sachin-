...
import { Lock, Send, CheckCircle2, ExternalLink } from 'lucide-react';
import SettingsLayout from '../../components/SettingsLayout';

export default function TelegramBotSettings() {
  return (
    <SettingsLayout activeTab="telegram">
      <div className="flex flex-col h-full">
        <div className="flex-1 overflow-y-auto p-6 max-w-[1150px] mx-auto w-full">
          ...
          <div className="bg-white border border-gray-200 rounded-none-none shadow-sm p-10 flex flex-col items-center text-center">
            <div className="w-14 h-14 rounded-none-none bg-sky-50 flex items-center justify-center mb-4">
              <Send className="w-7 h-7 text-sky-500" />
            </div>
            <h1 className="text-xl font-bold text-gray-800 mb-2">Telegram Bot</h1>
            <p className="text-sm text-gray-500 max-w-[1150px] leading-relaxed mb-6">
              Telegram bot token &amp; chat, with validate and test-message tools.
            </p>
            <div className="flex flex-col gap-2 mb-6 text-left w-full max-w-[1150px]">
              {['Validate the bot token', 'Send a test Telegram message'].map(f => (
                <div key={f} className="flex items-center gap-2.5 text-sm text-gray-700">
                  <CheckCircle2 className="w-4 h-4 text-blue-500 flex-shrink-0" /> {f}
                </div>
              ))}
            </div>
            <button onClick={() => window.open('/settings-telegram-workspace', '_blank')}
